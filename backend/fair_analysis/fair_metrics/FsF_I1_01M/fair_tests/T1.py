...
t1 = Test(
    name="Metadata is in a structured, parsable, and formal representation format (eg. JSON-LD, RDFa, RDF/XML etc.)",
    ...
    Key Steps to follow:-
    1. Check the metadata to see if it represented using a structured data format. Any sort of plain text or raw text like markdown etc. are not formal. The type of file source is also provided for reference.
    2. If a structured format is detected, check syntax.
    3. Finally check for formal repsentation by check the necessary vocabulary terms for either of the formal representation formats like `OWL`, `RDF` etc. are correctly used. Use the provided examples as reference.
